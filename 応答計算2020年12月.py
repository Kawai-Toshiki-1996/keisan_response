...
class Data_Handling(Wave_Spectrum):
    def __init__(self,column1,column2):
        ...
        self.amp1=np.reshape(strip[:,column1],(72,80)).T
        self.phase1=np.reshape(strip[:,column1+1],(72,80)).T
        self.amp2=np.reshape(strip[:,column2],(72,80)).T
        self.phase2=np.reshape(strip[:,column2+1],(72,80)).T
        self.z1= 1000*9.8065/Zc/10**6 if column1 == 17 else 1
        self.z2= 1000*9.8065/Zc/10**6 if column2 == 17 else 1
    ...
    def Response_calculation(self):
        X_Conjugate =self.amp1*(np.cos(-self.phase1*np.pi/180)+1j*np.sin(-self.phase1*np.pi/180))
        Y=self.amp2*(np.cos(self.phase2*np.pi/180)+1j*np.sin(self.phase2*np.pi/180)) 
        return X_Conjugate*Y*self.z1*self.z2
...
def make_dw_matrix():
    return np.abs(1.0-2.0*U/9.80655*np.expand_dims(omegalist,1)*np.cos(np.arange(0,360,360/n_angle)*np.pi/180.0))
