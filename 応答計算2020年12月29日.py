...
class Transformation:
    def __init__(self,Velocity=U,Omegalist=omegalist,N_angle=n_angle,N_lambda=n_lambda,Encounter_omega_list=encount_omega_2):
        ...
        self.Omega_interval=np.abs(Omegalist[0]-Omegalist[1])
        self.N_angle=N_angle
        self.N_lambda=N_lambda
        self.Encounter_omega_list=Encounter_omega_list
        self.N_pre_omega=len(Encounter_omega_list)#予め準備した出会い波周波数点の数
    ...
    def Interpolation(self,i='',column='',x):
        return np.array([(self.We_matrix[i+1,column]-x)/(self.We_matrix[i+1,column]-self.We_matrix[i,column]),(x-self.We_matrix[i,column])/(self.We_matrix[i+1,column]-self.We_matrix[i,column])])

    def TransMatrix(self):
        """間に入る出会い波周波数のリスト作成"""
        we_index_matrix=[]#3次元配列
        for column in range(self.N_angle):#角度ごと
            we_index_list=np.zeros((len(self.Encounter_omega_list),self.N_lambda))
            
            for i in range(self.N_lambda-1):
                """1つの周波数点が，どの出会い波周波数に変換されるか"""              
                applied_index=np.arange(self.N_pre_omega)[(self.We_matrix[i,column]-self.Encounter_omega_list)*(self.We_matrix[i+1,column]-self.Encounter_omega_list) <= 0]
                if len(applied_index) == 0 and np.max[self.We_matrix[i:i+2,column]) < np.max(self.Encounter_omega_list) :
                    """下側の点に外挿"""
                    [we_index_list[int(self.We_matrix[i,column]/interval),i],we_index_list[int(self.We_matrix[i,column]/interval),i+1]] =\
                        [we_index_list[int(self.We_matrix[i,column]/interval),i],we_index_list[int(self.We_matrix[i,column]/interval),i+1]]+\
                        self.Interpolation(i,column,self.Encounter_omega_list[int(self.We_matrix[i,column]/interval)])                                     
                    """上側の点に外挿"""
                    [we_index_list[int(self.We_matrix[i,column]/interval)+1,i],we_index_list[int(self.We_matrix[i,column]/interval)+1,i+1]]=\
                    [we_index_list[int(self.We_matrix[i,column]/interval)+1,i],we_index_list[int(self.We_matrix[i,column]/interval)+1,i+1]]+\
                    self.Interpolation(i,column,self.Encounter_omega_list[int(self.We_matrix[i,column]/interval)+1])
                else:
                    [we_index_list[applied_index,i],we_index_list[applied_index,i+1]]=\
                    [we_index_list[applied_index,i],we_index_list[applied_index,i+1]]+\
                    self.Interpolation(i,column,self.Encounter_omega_list[applied_index])
            
            we_index_matrix.append(we_index_list)
        return np.array(we_index_matrix)
        
    def Trans_to_Se_list(self,Se_matrix):
        Se_1D=np.zeros(self.N_pre_omega)
